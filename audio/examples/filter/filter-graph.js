...
function frequencyHandler(event, ui) {
    ...
    info.innerHTML =  (Math.floor(cutoff*100)/100) + " Hz";
...
function resonanceHandler(event, ui) {
    ...
    try{filter.Q.value = value;}catch(e){} // !! Q value not same as resonance...
    drawCurve();
}
...
function gainHandler(event, ui) {
    ...
    info.innerHTML =  (Math.floor(value*100)/100);
...
function initFilterGraph() {
    ...
    configureSlider("gain", gain, 0, 10, gainHandler);
    drawCurve();
}
